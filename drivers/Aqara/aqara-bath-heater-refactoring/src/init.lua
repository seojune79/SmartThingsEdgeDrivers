...
local function color_temp_handler(driver, device, value, zb_rx)
  ...
  log.warn(string.format("색온도: %d mired → %dK (표시)", mired, kelvin))
...
local function ac_code_attr_handler(driver, device, value, zb_rx)
  ...
  log.info(string.format(
    "[AC코드 수신] pwr=%X mode=%X fan=%X setpoint=%s | 0x%08X%08X",
    pwr, mode, fan_set, setpoint_str, hi32, lo32))
  ...
  if hi_valid and setpoint_raw ~= 0xFFFF then
    ...
    -- log for test
    log.warn(string.format("설정온도: %f℃", sp))
    log.warn(string.format("현재온도: %f℃", (hi32 & 0xFFFF)/100.0))
    --
  end
