...
function device_manager.presence_handler(driver, device, zone, evt_value)
  if not device.profile.components[COMP_PRESENCE] then return end
...
function device_manager.illuminance_handler(driver, device, zone, evt_value)
  device:emit_component_event(device.profile.components[COMP_ILLUMINANCE],
    capabilities.illuminanceMeasurement.illuminance(evt_value))
end
